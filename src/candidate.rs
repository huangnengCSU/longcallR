...
impl SNPFrag {
    pub fn get_candidate_snps(
        ...
        let use_annotation = if exon_region_vec.len() > 0 { true } else { false };
        ...
        for bfidx in 0..pileup.len() {
            ...
            let valid_alleles = ['A', 'C', 'G', 'T', 'a', 'c', 'g', 't'];
            if !valid_alleles.contains(&allele1) || !valid_alleles.contains(&allele2) {
                println!("Error: unknown allele: {}:{} {},{}", profile.region.chr, position, allele1, allele2);
                position += 1;
                continue;
            }

            if allele1 == bf.ref_base && allele2 != bf.ref_base {
                if (depth >= 200 && allele2_cnt < somatic_allele_cnt_cutoff) || (depth < 200 && allele2_freq < somatic_allele_frac_cutoff) {
                    position += 1;
                    continue;
                }
            } else if allele2 == bf.ref_base && allele1 != bf.ref_base {
                if (depth >= 200 && allele1_cnt < somatic_allele_cnt_cutoff) || (depth < 200 && allele1_freq < somatic_allele_frac_cutoff) {
                    position += 1;
                    continue;
                }
            } else if allele1 != bf.ref_base && allele2 != bf.ref_base {
                if (depth >= 200 && allele1_cnt < somatic_allele_cnt_cutoff && allele2_cnt < somatic_allele_cnt_cutoff) || (depth < 200 && allele1_freq < somatic_allele_frac_cutoff && allele2_freq < somatic_allele_frac_cutoff) {
                    position += 1;
                    continue;
                }
            }
            ...
            if (allele1 != bf.ref_base && bf.d >= allele1_cnt) || (allele2 != bf.ref_base && bf.d >= allele2_cnt) {
                position += 1;
                continue;
            }
            let depth_include_intron = bf.get_depth_include_intron();
            if (allele1_cnt + allele2_cnt) as f32 / (depth_include_intron as f32) < min_allele_freq_include_intron {
                position += 1;
                continue;
            }
            let mut allele1_quals: Vec<u8> = bf.get_allele_baseq(allele1);
            let mut allele2_quals: Vec<u8> = bf.get_allele_baseq(allele2);
            // filter if all alt alleles have low base quality
            if allele1 != bf.ref_base {
                let allele1_bq_pass_cnt = allele1_quals.iter().filter(|&&bq| bq >= min_baseq).count();
                if allele1_cnt > 0 && allele1_bq_pass_cnt < 2 {
                    position += 1;
                    continue;
                }
            } else if allele2 != bf.ref_base {
                let allele2_bq_pass_cnt = allele2_quals.iter().filter(|&&bq| bq >= min_baseq).count();
            ...
            if use_strand_bias {
                ...
                if variant_allele.len() > 0 {
                    ...
                    let (freads, breads) = (bf.forward_cnt as i32, bf.backward_cnt as i32);
                    for allele_base in variant_allele.iter() {
                        let [fcnt, bcnt] = bf.get_allele_base_strands(*allele_base);
    ...
    pub fn get_candidate_snps2(
        &mut self,
        profile: &Profile,
        pre_hetvar_sites: &Vec<usize>,
        pre_homvar_sites: &Vec<usize>,
        platform: &Platform,
        exon_region_vec: Vec<Interval<usize, u8>>,
        min_allele_freq: f32,
        min_qual: u32,
        ...
        for bfidx in 0..pileup.len() {
            ...
            if !pre_hetvar_sites.contains(&(position as usize)) && !pre_homvar_sites.contains(&(position as usize)) {
                if allele1 == bf.ref_base && allele2 != bf.ref_base {
                ...
                // Low QUAL sites are filtered out
                if variant_quality < min_qual as f64 && !pre_hetvar_sites.contains(&(position as usize)) && !pre_homvar_sites.contains(&(position as usize)) {
                    position += 1;
                    continue;
                }
            ...
            if candidate_snp.variant_type == 2 || pre_homvar_sites.contains(&(position as usize)) {
                // hom_var
                if allele1 != bf.ref_base && allele2 != bf.ref_base && allele1_freq >= min_allele_freq && allele2_freq >= min_allele_freq {
                    candidate_snp.variant_type = 3; // triallelic SNP
                    candidate_snp.genotype = -1;
            ...
            if candidate_snp.variant_type == 1 || pre_hetvar_sites.contains(&(position as usize)) {
                // het_var
                if allele1 != bf.ref_base && allele2 != bf.ref_base {
                    candidate_snp.variant_type = 3; // triallelic SNP
                    candidate_snp.genotype = -1;
