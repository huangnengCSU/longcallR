...
impl SNPFrag {
    pub fn get_candidate_snps(
        ...
        for bfidx in 0..pileup.len() {
            ...
            if genotype_prob[0] > genotype_prob[1] && genotype_prob[0] > genotype_prob[2] {
                ...
                if (candidate_snp.reference == 'A' && (candidate_snp.alleles[0] == 'G' || candidate_snp.alleles[1] == 'G')) || (candidate_snp.reference == 'T' && (candidate_snp.alleles[0] == 'C' || candidate_snp.alleles[1] == 'C')) {
                    // A-G sites stored in ase snps
                    if allele1 != candidate_snp.reference {
                    ...
                    } else if allele2 != candidate_snp.reference {
    ...
    pub fn get_fragments_with_ase(&mut self, bam_path: &str, region: &Region) {
        ...
        while let Some(result) = bam_reader.read(&mut record) {
            ...
            if fragment.read_id == "SRR18130587.2326699".to_string() {
    ...
    pub fn phase_ase_hete_snps(
        ...
        for i in self.ase_snps.iter() {
            println!("ase snp: {:?}", self.candidate_snps[*i]);
        }
    ...
    pub fn assign_reads(&mut self, read_assignment_cutoff: f64) -> HashMap<String, i32> {
        ...
        for k in 0..self.fragments.len() {
            ...
            let mut ps: Vec<i32> = Vec::new();
            let mut probs: Vec<f64> = Vec::new();
            if self.fragments[k].read_id == "SRR18130587.2326699".to_string() {
...
fn calculate_prob_somatic(hap1_ref_baseqs: &Vec<u8>, hap1_alt_baseqs: &Vec<u8>, hap2_ref_baseqs: &Vec<u8>, hap2_alt_baseqs: &Vec<u8>, purity: f64) -> (AlleleClass, AlleleClass) {
