...
pub struct SNPFrag {
    ...
    // pub high_qual_hete_snps_index: Vec<usize>,
    // // index of high quality heterozygous SNPs in self.snps
}
...
impl SNPFrag {
    ...
    pub fn filter_fp_snps(&mut self, strand_bias_threshold: f32, phase_score_cutoff: Option<f64>) {
        ...
        *? Filter false positive SNPs, which may be cause by strand bias or in a dense cluster of variants (100bp length has over 3 snps).
        ...
        for i in 0..self.snps.len() {
            ...
            if phase_score_cutoff.is_some() {
                if snp.phase_score < phase_score_cutoff.unwrap() {
                    println!("filter candidate snp by phase score: {}, {}", snp.pos, snp.phase_score);
                    continue;
                }
            }
    ...
    pub fn phase(&mut self, max_enum_snps: usize, random_flip_fraction: f32) {
        ...
        if self.snps.len() <= max_enum_snps {
            ...
            let num_flip_haplotype = (self.haplotype.len() as f32 * random_flip_fraction) as usize;
            let num_flip_haplotag = (self.haplotag.len() as f32 * random_flip_fraction) as usize;
            let mut rng = rand::thread_rng();
            let selected_flip_haplotype: Vec<_> = (0..self.haplotype.len()).collect::<Vec<_>>().choose_multiple(&mut rng, num_flip_haplotype).cloned().collect();
            for ti in selected_flip_haplotype.iter() {
                self.haplotype[*ti] = self.haplotype[*ti] * (-1);
            }
            let selected_flip_haplotag: Vec<_> = (0..self.haplotag.len()).collect::<Vec<_>>().choose_multiple(&mut rng, num_flip_haplotag).cloned().collect();
            for tk in selected_flip_haplotag.iter() {
                self.haplotag[*tk] = self.haplotag[*tk] * (-1);
            }
            let prob = self.cross_optimize();
            if prob > largest_prob {
                largest_prob = prob;
            ...
            for k in self.snp_cover_fragments[i].iter() {
                ...
                for fe in self.fragments[*k].list.iter() {
                    if fe.snp_idx == i {
                        if fe.base != '-' {
                            // ignore intron
                            if self.fragments[*k].haplotag == 1 {
                                num_hap1 += 1;
                            } else if self.fragments[*k].haplotag == 2 {
                                num_hap2 += 1;
                            }
                        }
                        if fe.p != 0 {
    ...
    pub fn assign_reads(&mut self, read_assignment_cutoff: f64) -> HashMap<String, i32> {
        ...
        for k in 0..self.haplotag.len() {
            ...
            if SNPFrag::cal_log_sigma_delta(sigma_k, &delta, &ps, &probs) - SNPFrag::cal_log_sigma_delta(sigma_k * (-1), &delta, &ps, &probs) > read_assignment_cutoff {
                if sigma_k == 1 {
                    self.fragments[k].haplotag = 1;
                    read_assignments.insert(self.fragments[k].read_id.clone(), 1);
                } else {
                    self.fragments[k].haplotag = 2;
